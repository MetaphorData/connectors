...
def test_select():
    assert_table_lineage_equal("SELECT col1 FROM tab1", {"tab1"}, None)


def test_select_with_schema():
    assert_table_lineage_equal("SELECT col1 FROM schema1.tab1", {"schema1.tab1"}, None)


def test_select_with_schema_and_database():
    assert_table_lineage_equal(
        "SELECT col1 FROM db1.schema1.tbl1", {"db1.schema1.tbl1"}, None
    )
...
def test_select_value():
    assert_table_lineage_equal("SELECT 1", None, None)


def test_select_function():
    assert_table_lineage_equal("SELECT NOW()", None, None)


def test_select_trim_function_with_from_keyword():
    assert_table_lineage_equal("SELECT trim(BOTH '  ' FROM '  abc  ')", None, None)


def test_select_trim_function_with_from_keyword_from_source_table():
    assert_table_lineage_equal(
        "SELECT trim(BOTH '  ' FROM col1) FROM tab1", {"tab1"}, None
    )


def test_select_with_where():
    assert_table_lineage_equal(
        "SELECT * FROM tab1 WHERE col1 > val1 AND col2 = 'val2'",
        {"tab1"},
        None,
    )
...
def test_select_keyword_as_column_alias():
    ...
    assert_table_lineage_equal('SELECT 1 "as" FROM tab1', {"tab1"}, None)
...
def test_select_with_table_alias():
    assert_table_lineage_equal("SELECT 1 FROM tab1 AS alias1", {"tab1"}, None)


def test_select_count():
    assert_table_lineage_equal("SELECT count(*) FROM tab1", {"tab1"}, None)


def test_select_parenthesized_from_table():
    sql = "SELECT * FROM (tab1)"
    assert_table_lineage_equal(sql, {"tab1"}, None)
...
FROM (tab1
    ...
    assert_table_lineage_equal(sql, {"tab1", "tab2"}, None)
...
FROM ((tab1
    ...
    assert_table_lineage_equal(sql, {"tab1", "tab2", "tab3"}, None)


def test_select_subquery():
    assert_table_lineage_equal(
        "SELECT col1 FROM (SELECT col1 FROM tab1) dt", {"tab1"}, None
    )
    # with an extra space
    assert_table_lineage_equal(
        "SELECT col1 FROM ( SELECT col1 FROM tab1) dt", {"tab1"}, None
...
def test_select_subquery_without_alias():
    ...
    assert_table_lineage_equal(
        "SELECT col1 FROM (SELECT col1 FROM tab1)", {"tab1"}, None
    )
...
def test_select_inner_join():
    assert_table_lineage_equal(
        "SELECT * FROM tab1 INNER JOIN tab2", {"tab1", "tab2"}, None
    )


def test_select_join():
    assert_table_lineage_equal("SELECT * FROM tab1 JOIN tab2", {"tab1", "tab2"}, None)


def test_select_left_join():
    assert_table_lineage_equal(
        "SELECT * FROM tab1 LEFT JOIN tab2", {"tab1", "tab2"}, None
    )
...
def test_select_right_join():
    assert_table_lineage_equal(
        "SELECT * FROM tab1 RIGHT JOIN tab2", {"tab1", "tab2"}, None
    )


def test_select_full_outer_join():
    assert_table_lineage_equal(
        "SELECT * FROM tab1 FULL OUTER JOIN tab2", {"tab1", "tab2"}, None
    )


def test_select_cross_join():
    assert_table_lineage_equal(
        "SELECT * FROM tab1 CROSS JOIN tab2", {"tab1", "tab2"}, None
    )


def test_select_cross_join_with_on():
    assert_table_lineage_equal(
        "SELECT * FROM tab1 CROSS JOIN tab2 ON tab1.col1 = tab2.col2",
        {"tab1", "tab2"},
        None,
    )
...
def test_select_join_in_ansi89_syntax():
    assert_table_lineage_equal("SELECT * FROM tab1 a, tab2 b", {"tab1", "tab2"}, None)


def test_select_join_in_ansi89_syntax_with_subquery():
    assert_table_lineage_equal(
        "SELECT * FROM (SELECT * FROM tab1) a, (SELECT * FROM tab2) b",
        {"tab1", "tab2"},
        None,
    )
...
def test_select_group_by():
    assert_table_lineage_equal(
        "SELECT col1, col2 FROM tab1 GROUP BY col1, col2", {"tab1"}, None
    )


def test_select_group_by_ordinal():
    assert_table_lineage_equal(
        "SELECT col1, col2 FROM tab1 GROUP BY 1, 2", {"tab1"}, None
    )


def test_select_from_values():
    assert_table_lineage_equal("SELECT * FROM (VALUES (1, 2))", None, None)
...
def test_select_from_values_with_alias():
    assert_table_lineage_equal(
        "SELECT * FROM (VALUES (1, 2)) AS t(col1, col2)", None, None
    )
...
def test_non_reserved_keyword_as_source():
    assert_table_lineage_equal("SELECT col1, col2 FROM segment", {"segment"}, None)
