...
def test_percentile():
    ...
    dataset2 = make_dataset_with_usage([10, 20, 30, 40, 50])
    dataset3 = make_dataset_with_usage([100, 200, 300, 400, 500])
    ...
    assert dataset1.usage.query_counts.last7_days == QueryCount(count=2)
    assert dataset1.usage.query_counts.last30_days == QueryCount(count=3)

    assert dataset2.usage.query_counts.last24_hours == QueryCount(
        count=10, percentile=0.3333333333333333
    )
    assert dataset2.usage.query_counts.last7_days == QueryCount(count=20)
    assert dataset2.usage.query_counts.last30_days == QueryCount(count=30)

    assert dataset3.usage.query_counts.last24_hours == QueryCount(
        count=100, percentile=0.6666666666666666
    )
    assert dataset3.usage.query_counts.last7_days == QueryCount(count=200)
    assert dataset3.usage.query_counts.last30_days == QueryCount(count=300)
